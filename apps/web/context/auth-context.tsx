...
import { createBrowserClient } from "@gezgin/supabase-web";
import { getProfileById } from "@gezgin/supabase-queries";
import type { Database, Profile } from "@gezgin/types";

export type WebSupabaseClient = ReturnType<typeof createBrowserClient>;
...
interface AuthContextType {
  ...
  supabase: WebSupabaseClient | null;
}
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  const [supabaseClient, setSupabaseClient] =
    useState<WebSupabaseClient | null>(null);
  const [user, setUser] = useState<User | null>(null);
  const [session, setSession] = useState<Session | null>(null);
  const [profile, setProfile] = useState<Profile | null>(null);
  const [isLoading, setIsLoading] = useState<boolean>(true);

  // Initialize Supabase browser client safely
  useEffect(() => {
    try {
      ...
      console.warn(
        "Supabase credentials not configured in environment. Auth runs in mock/offline mode.",
        err,
      );
      setIsLoading(false);
    }
  ...
  const loadProfile = useCallback(
    async (client: WebSupabaseClient, userId: string) => {
      try {
